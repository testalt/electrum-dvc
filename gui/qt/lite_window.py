...
try:
    ...
    print "You need to have PyQT installed to run Electrum-dvc in graphical mode."
...
from electrum_dvc.util import get_resource_path as rsrc
...
import re
import time
from electrum_dvc.wallet import Wallet, WalletStorage
import webbrowser
import history_widget
import receiving_widget
from electrum_dvc import util
import datetime

from electrum_dvc.version import ELECTRUM_VERSION as electrum_version
from electrum_dvc.util import format_satoshis, age
...
class MiniWindow(QDialog):
    ...
    def __init__(self, actuator, expand_callback, config):
        ...
        self.address_input.setPlaceholderText(_("Enter a devcoin address or contact"))
        ...
        self.setWindowTitle("Electrum-dvc")
    ...
    def set_balances(self, btc_balance):
        """Set the devcoin balance and update the amount label accordingly."""
        ...
        self.setWindowTitle("Electrum-dvc %s - %s %s" % (electrum_version, amount, unit))

    def amount_input_changed(self, amount_text):
        """Update the number of devcoins displayed."""
    ...
    def check_button_status(self):
        """Check that the devcoin address is valid and that something
    ...
    def the_website(self):
        webbrowser.open("http://dvc.electrum-alt.org")
...
class BalanceLabel(QLabel):
    ...
    def set_balance_text(self, amount, unit, quote_text):
        """Set the amount of devcoins in the gui."""
        if self.state == self.SHOW_CONNECTING:
...
class ReceivePopup(QDialog):
    ...
    def setup(self, address):
        label = QLabel(_("Copied your devcoin address to the clipboard!"))
        ...
        self.setWindowTitle("Electrum-dvc - " + _("Receive devcoin payment"))
...
class MiniActuator:
    ...
    def send(self, address, amount, parent_window):
        """Send devcoins to the target address."""
