...
from electrum_dvc.util import print_error
from electrum_dvc.bitcoin import is_valid
from electrum_dvc import WalletStorage, Wallet

Gdk.threads_init()
APP_NAME = "Electrum-dvc"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from electrum_dvc.util import format_satoshis, parse_URI
from electrum_dvc.network import DEFAULT_SERVERS
from electrum_dvc.bitcoin import MIN_RELAY_TX_FEE
...
def restore_create_dialog():
    ...
    dialog = Gtk.Dialog("electrum-dvc", parent=None, 
                        flags=Gtk.DialogFlags.MODAL,
...
def run_network_dialog( network, parent ):
    ...
    add_help_button(host_box, 'The name, port number and protocol of your Electrum-dvc server, separated by a colon. Example: "ecdsa.org:50002:s". Some servers allow you to connect through http (port 80) or https (port 443)')
    host_box.show()
...
class ElectrumWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Electrum-dvc ' + self.wallet.electrum_version + '  -  ' + self.config.path
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid devcoin address:\n"+to_address)
            return
        ...
        #@todo devcoin electrum-ltc modifies this...
